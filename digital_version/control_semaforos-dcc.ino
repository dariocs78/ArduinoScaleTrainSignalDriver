...
// VERSION PARA MAQUETAS DIGITALES / DIGITAL LAYOUTS VERSION
...
void setup() {
  ...
  // COMUNICACIONES / COMMUNICATIONS
    ...
    Serial.begin(9600);   // Ha de ser igual al emisor / Must match emitter's rate
  ...
  // ASPECTO INICIAL / INITIAL ASPECT
...
void loop() {
  ...
  // Comando del decoder / Command from decoder

    control_recepcion = recepcion_comando((byte*)&comando_recibido, sizeof(comando_recibido));


    if (control_recepcion == true)
    {
      actualiza_comando();
...
bool recepcion_comando(byte *puntero_estructura, int longitud)    // Funcion para recibir una estructura por puerto serie / Function to receive an structure through serial port
{
  ...
  {
    Serial.readBytes(puntero_estructura, longitud);
    return true;
  }

}

void actualiza_comando()                                          // Funcion para actualizar el array de comandos / Function to update the command array
{
    for (uint8_t i = 0; i < Numero_semaforos; i++)
    {
        if (comando_recibido[i].estado != comando[i])
        {
